                            ...
                            @forelse($formulaAramas as $index => $aroma)
                                <div class="border-2 border-gray-200 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition bg-[#FCF3EE]/30 hover:border-[#FF6BD0]">
                                    ...
                                    <div class="flex items-center space-x-2 w-full sm:w-auto">
                                        ...
                                        <input type="number" name="takaran[{{ $aroma->formula_id }}]" value="0" min="0" max="200" 
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function getCapacity() {
                return parseInt(sizeSelect.value.replace('ml', '')) || 50;
            }
            ...
            function updateLiquid() {
                ...
                checkboxes.forEach((cb, i) => {
                    if (cb.checked) {
                        const val = parseInt(takaranInputs[i].value) || 0;
                        totalMl += val;
                        lastColor = cb.getAttribute('data-color');
                    }
                });

                // Validasi agar tidak luber melewati kapasitas botol
                if (totalMl > maxCapacity) {
                    alert(`Takaran melebihi kapasitas botol (${maxCapacity}ml)!`);
                }
                ...
                if (totalMl === 0) {
                    ...
                    const fillPercentage = Math.min((totalMl / maxCapacity) * 100, 100);
                    liquidLayer.style.height = `${fillPercentage}%`;
                    liquidLayer.style.backgroundColor = lastColor;
                }
            }
            ...
            checkboxes.forEach((cb, i) => {
                cb.addEventListener('change', function() {
                    if (!this.checked) {
                        takaranInputs[i].value = 0;
                    } else if (this.checked && (parseInt(takaranInputs[i].value) || 0) === 0) {
                        takaranInputs[i].value = 5; 
                    }
                    updateLiquid();
                });
            });
